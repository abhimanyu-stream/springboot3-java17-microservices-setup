package com.stream.product.catalogue.wishlist.factory;

import com.stream.product.catalogue.wishlist.dto.ProductRequest;
import com.stream.product.catalogue.wishlist.model.BeautyAndPersonalCareProduct;

public interface BeautyAndPersonalCareProductFactory {

    public abstract BeautyAndPersonalCareProduct createBeautyAndPersonalCareProduct(ProductRequest productRequest);
}
